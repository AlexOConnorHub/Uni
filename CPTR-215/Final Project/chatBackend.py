...
def login():
    print(chat.replacePassphrase(bottle.request.forms.get('uname'),
...
def signup():
    bottle.redirect(chat.makeLogin(bottle.request.forms.get('uname'), bottle.request.forms.get('pass1'), bottle.request.forms.get('pass2')))
...
def makeChat(message):
    ...
    state = {"state":(valid[message['sender']] == message['cookie']) and chat.isClean(message['message'])}
    if state['state']:
...
def saveFile(request):
    ...
    if (valid[request.forms.uname] == request.forms.cookie):
        ...
        for end in exts:
            if os.path.exists(public + 'images/profile/' + name + end):
                os.remove(public + 'images/profile/' + name + end)
